...
# 사이트로부터 받아온 값
team_info = requests.get(URL, params=payload, timeout=10)

# 파이썬에서 다루기 위해 파이썬 문법에 맞게 변환
data = team_info.json()
...
for k, v in info.items():
    ...
    if "strTeam" in info and k == "strTeam":
        print(f"구단명 : {v} ({info["strTeamAlternate"]})")

    elif  "strLeague" in info and k ==  "strLeague":
        print(f"소속 리그 : {v}")

    elif "intFormedYear" in info and k == "intFormedYear":
        print(f"창단 연도 : {v}")

    elif  "strStadium" in info and k ==  "strStadium":
        print(f"홈구장 : {v}")

    elif  "strLocation" in info and k ==  "strLocation":
        print(f"연고지 : {v}")
